...
// --- COMPONENT: RESPONSIVE STRATEGIC TIMELINE ---
const MermaidChart = ({ chartCode, roadmapData, lang, t }) => {
...
export default function VectorAI() {
  ...
  const [selectedModel, setSelectedModel] = useState(DEFAULT_AGENT || "gemini-2.5-flash-lite");
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event) => setFormData(prev => ({ ...prev, context: prev.context + "\n" + event.results[0][0].transcript }));
    recognition.start();
  };
  ...
  const toggleTag = (tagId) => {
    setFormData(prev => {
      ...
      return tags.includes(tagId) ? { ...prev, activeTags: tags.filter(t => t !== tagId) } : { ...prev, activeTags: [...tags, tagId] };
    });
  };
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4 animate-slideUp print:hidden">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar">
             <button onClick={() => setLang(lang === 'en' ? 'my' : 'en')} className="flex items-center flex-shrink-0 gap-2 px-3 py-2 bg-gray-900 border border-gray-800 rounded-lg hover:border-emerald-500/50">
                <Globe size={14} className="text-blue-400"/><span className="text-[10px] font-bold uppercase text-gray-300">{lang === 'en' ? 'ENG' : 'BM'}</span>
             </button>
             <button onClick={() => setPrivacyMode(!privacyMode)} className={`flex items-center flex-shrink-0 gap-2 px-3 py-2 rounded-lg border transition-all ${privacyMode ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-400" : "bg-gray-900 border-gray-800 text-gray-500 hover:text-gray-300"}`}>
              {privacyMode ? <EyeOff size={14}/> : <Eye size={14}/>}<span className="text-[10px] font-bold uppercase">{privacyMode ? t.privacy_on : t.privacy_off}</span>
            </button>
            <div className="flex items-center flex-shrink-0 gap-2 bg-gray-900 border border-gray-800 px-3 py-2 rounded-lg group hover:border-emerald-500/50 transition-colors">
              <Cpu size={14} className="text-emerald-500" />
              <div className="flex flex-col"><span className="text-[10px] text-gray-500 font-bold uppercase leading-none">{t.agent_model}</span><select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} className="bg-transparent text-xs text-white font-mono outline-none cursor-pointer appearance-none min-w-[100px]">{AI_MODELS.map(m => <option key={m.id} value={m.id} className="bg-gray-900 text-gray-300">{m.name}</option>)}</select></div>
        ...
        {(status === 'input' || status === 'error') && (
          // FIX: Add padding-bottom to prevent content being hidden by the sticky action bar
          <div className="space-y-8 animate-slideUp pb-28"> 
            
            {status === 'error' && <div className="bg-red-900/20 border border-red-500/50 p-4 rounded-lg flex items-center gap-3"><AlertTriangle className="text-red-400" size={20} /><div><h4 className="text-red-400 font-bold text-sm">{t.error_title}</h4></div><button onClick={handleAnalyze} className="ml-auto bg-red-500/20 hover:bg-red-500/30 text-red-300 px-3 py-1 rounded text-xs border border-red-500/30">Retry</button></div>}

            {/* SCENARIO SELECTOR */}
            <div className="flex flex-col gap-2"><span className="text-[10px] uppercase font-bold text-gray-500 ml-1">{t.scenarios_label}</span><div className="bg-gray-900/50 p-1 rounded-xl border border-gray-800 flex overflow-x-auto">{Object.entries(SCENARIOS).map(([key, info]) => <button key={key} onClick={() => loadScenario(key)} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-sm font-medium transition-all ${JSON.stringify(formData.context) === JSON.stringify(info.data.context) ? "bg-emerald-600/20 text-emerald-400 border border-emerald-500/30" : "text-gray-500 hover:text-gray-300 hover:bg-gray-800"}`}>{info.icon}{info.label}</button>)}</div></div>

            {/* FORM GRID */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-1 space-y-4"><div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl"><h3 className="text-gray-400 text-xs font-bold uppercase mb-4 flex items-center gap-2"><DollarSign size={14}/> {t.core_financials}</h3><div className="space-y-4"><div><label className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">{t.label_salary}</label><input name="salary" value={formData.salary} onChange={handleInputChange} className={`w-full bg-transparent border-b border-gray-700 py-2 text-xl font-mono text-white focus:border-emerald-500 outline-none transition ${blurClass}`} /></div><div><label className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">{t.label_expenses}</label><input name="expenses" value={formData.expenses} onChange={handleInputChange} className={`w-full bg-transparent border-b border-gray-700 py-2 text-xl font-mono text-white focus:border-emerald-500 outline-none transition ${blurClass}`} /></div><div><label className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">{t.label_goal}</label><select name="goalType" value={formData.goalType} onChange={handleInputChange} className="w-full bg-transparent border-b border-gray-700 py-2 text-sm text-emerald-400 outline-none cursor-pointer"><option value="property_wealth">{t.goals.property}</option><option value="debt_clearance">{t.goals.debt}</option><option value="retirement">{t.goals.fire}</option></select></div></div></div></div>
              <div className="lg:col-span-2 space-y-4"><div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl"><h3 className="text-gray-400 text-xs font-bold uppercase mb-4 flex items-center gap-2"><Zap size={14}/> {t.life_context}</h3><div className="flex flex-wrap gap-2">{CONTEXT_TAGS.map(tag => <button key={tag.id} onClick={() => toggleTag(tag.id)} className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${formData.activeTags?.includes(tag.id) ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-400" : "bg-gray-800 border-gray-700 text-gray-500 hover:border-gray-500"}`}>{tag.label}</button>)}</div></div><div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl relative group focus-within:border-emerald-500/50 transition-colors"><div className="absolute top-5 right-5 flex items-center gap-2"><button onClick={handleVoiceInput} className={`p-2 rounded-full transition-all ${isListening ? "bg-red-500/20 text-red-400 animate-pulse" : "bg-gray-800 text-gray-400 hover:text-emerald-400"}`} title={lang==='my' ? "Cakap Melayu" : "Speak English"}>{isListening ? <MicOff size={16}/> : <Mic size={16}/>}</button><Brain size={18} className="text-gray-600" /></div><h3 className="text-gray-400 text-xs font-bold uppercase mb-2">{t.brain_dump_title}</h3><textarea name="context" value={formData.context} onChange={handleInputChange} rows={4} className="w-full bg-transparent border-none p-0 text-sm font-mono text-gray-300 focus:ring-0 resize-none leading-relaxed placeholder-gray-700" placeholder={t.brain_dump_placeholder} /></div></div>
            </div>

            {/* --- FIX: STICKY ACTION BAR for MOBILE --- */}
            <div className="fixed bottom-0 left-0 right-0 z-50 p-4 bg-gray-900/80 border-t border-gray-800 backdrop-blur-sm md:static md:p-0 md:bg-transparent md:border-none md:backdrop-blur-none">
              <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-1 rounded-2xl border border-gray-800 md:bg-none md:border-none md:p-0">
                <div className="flex items-center gap-4 p-2 md:p-0">
                  <div className="flex-1 md:px-4">
                    <label className="hidden md:block text-[10px] text-gray-500 uppercase font-bold">{t.specific_question}</label>
                    <input name="specificQuestion" value={formData.specificQuestion} onChange={handleInputChange} className="w-full bg-transparent border-none p-0 text-white focus:ring-0 placeholder-gray-500 font-medium md:placeholder-gray-600" placeholder={t.specific_question + "..."} />
                  </div>
                  <button onClick={handleAnalyze} className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 md:px-8 py-4 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-emerald-900/20 hover:scale-[1.02]"><Bot size={20} /> <span className="hidden md:inline">{t.analyze_btn}</span></button>
                </div>
              </div>
            </div>
        ...
        {status === 'analyzing' && <FinancialLoader />}
        ...
        {status === 'results' && aiResponse && (
          <div className="space-y-8 animate-zoomIn print:space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:grid-cols-2">
                {aiResponse.actions.map((action, idx) => (
                  <div key={idx} className={`p-6 rounded-2xl border transition-all hover:scale-[1.02] cursor-default print:bg-white print:border-black print:text-black ${action.type === 'critical' ? 'bg-red-500/10 border-red-500/30' : action.type === 'warning' ? 'bg-amber-500/10 border-amber-500/30' : 'bg-gray-800/50 border-gray-700'}`}>
                    <div className="flex justify-between mb-3"><span className={`text-[10px] font-bold uppercase px-2 py-1 rounded print:border print:border-black ${action.type === 'critical' ? 'bg-red-500/20 text-red-400 print:text-red-700' : action.type === 'warning' ? 'bg-amber-500/20 text-amber-400 print:text-amber-700' : 'bg-blue-500/20 text-blue-400 print:text-blue-700'}`}>{action.type}</span></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 print:block">
              <div className="lg:col-span-2 print:mb-6">{aiResponse.mermaidCode && <MermaidChart chartCode={aiResponse.mermaidCode} roadmapData={aiResponse.roadmap} lang={lang} t={t} />}</div>
