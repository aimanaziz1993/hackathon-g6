...
import { 
  ...
  ShieldCheck, Loader2, RefreshCw, Eye, EyeOff, Mic, MicOff, Printer
} from 'lucide-react';
    ...
    <div className="w-full bg-gray-900 border border-gray-700 rounded-xl mt-8 animate-fadeIn overflow-hidden shadow-2xl print:border-gray-300 print:bg-white">
      <div className="p-4 border-b border-gray-800 bg-gray-900/50 flex justify-between items-center print:bg-gray-100 print:border-gray-300">
        <h4 className="text-sm font-bold text-gray-300 uppercase tracking-wider flex items-center gap-2 print:text-black">
          <Activity size={16} className="text-emerald-500" /> Strategic Timeline
        </h4>
...
export default function VectorAI() {
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event) => {
      ...
      setFormData(prev => ({ 
        ...prev, 
        context: prev.context + "\n" + transcript // Append to existing context
      }));
    };
    recognition.start();
  };
  ...
  const handleAnalyze = async () => {
    ...
    while (attempt < 3) {
      try {
        attempt++;
        if (attempt > 1) setLoadingMsg(`Server overloaded. Retrying (${attempt}/3)...`);
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col md:flex-row gap-3">
             ...
             <button 
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg border transition-all ${
                privacyMode ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-400" : "bg-gray-900 border-gray-800 text-gray-500 hover:text-gray-300"
              }`}
            >
              {privacyMode ? <EyeOff size={14}/> : <Eye size={14}/>}
              <span className="text-[10px] font-bold uppercase">{privacyMode ? "Stealth On" : "Stealth Off"}</span>
            </button>
            ...
            <div className="hidden md:flex items-center gap-2 px-3 py-2 bg-gray-900 border border-gray-800 rounded-lg">
              ...
              <span className="text-[10px] text-gray-400 font-medium">
                {import.meta.env.VITE_GEMINI_API_KEY ? "ENV KEY LOADED" : "NO KEY"}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-8 animate-slideUp">
            ...
            {status === 'error' && (
              <div className="bg-red-900/20 border border-red-500/50 p-4 rounded-lg flex items-center gap-3">
                 ...
                 <div><h4 className="text-red-400 font-bold text-sm">Analysis Failed (503 Overloaded)</h4></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                       ...
                       <select name="goalType" value={formData.goalType} onChange={handleInputChange} className="w-full bg-transparent border-b border-gray-700 py-2 text-sm text-emerald-400 outline-none cursor-pointer">
                        <option value="property_wealth">Build Property Portfolio</option>
                        <option value="debt_clearance">Kill All Debt Fast</option>
                        <option value="retirement">FIRE (Retire Early)</option>
                       </select>
                    </div>
                  </div>
                </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl">
                   <h3 className="text-gray-400 text-xs font-bold uppercase mb-4 flex items-center gap-2"><Zap size={14}/> Life Context</h3>
                ...
                <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl relative group focus-within:border-emerald-500/50 transition-colors">
                  <div className="absolute top-5 right-5 flex items-center gap-2">
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-full transition-all ${isListening ? "bg-red-500/20 text-red-400 animate-pulse" : "bg-gray-800 text-gray-400 hover:text-emerald-400"}`}
                      title="Speak to enter data"
                    >
                  ...
                  <textarea 
                    ...
                    placeholder="Click the mic and just vent..."
                  />
                </div>
              </div>
            </div>
            ...
            <div className="mt-8">
              <button onClick={() => setShowPromptPreview(!showPromptPreview)} className="flex items-center gap-2 text-xs font-mono text-gray-500 hover:text-emerald-400 mb-3 transition-colors ml-1">
                {showPromptPreview ? <ChevronUp size={14} /> : <Terminal size={14} />}{showPromptPreview ? "Hide Terminal" : "View Live Prompt Construction"}
              </button>
              ...
              <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-1 rounded-2xl border border-gray-800">
                <div className="flex items-center gap-4 p-2">
                  ...
                  <button onClick={handleAnalyze} className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-4 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-emerald-900/20 hover:scale-[1.02]"><Bot size={20} /> Analyze</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {status === 'analyzing' && (
          <div className="flex flex-col items-center justify-center py-24 animate-pulse">
            ...
            <div className="space-y-2 text-center"><h3 className="text-2xl font-bold text-white">Vector is Thinking</h3><p className="text-xs font-mono text-emerald-500 uppercase tracking-widest">Agent: {selectedModel}</p><div className="text-sm text-gray-500 font-mono mt-2 flex items-center justify-center gap-2"><Loader2 size={14} className="animate-spin" />{loadingMsg}</div></div>
          </div>
        )}
        ...
        {status === 'results' && aiResponse && (
          <div className="space-y-8 animate-zoomIn print:space-y-4">
            <div className="flex justify-between items-center mb-4 print:hidden">
              <button onClick={() => setStatus('input')} className="text-xs text-gray-500 hover:text-white flex items-center gap-2">← Refine Data</button>
              <div className="flex items-center gap-4">
                <button onClick={handlePrint} className="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1"><Printer size={12}/> Print Strategy</button>
            ...
            <div className="bg-gradient-to-r from-emerald-900/30 to-gray-900 border border-emerald-500/30 p-8 rounded-3xl relative overflow-hidden print:bg-white print:border-black print:text-black">
              ...
              <div className="relative z-10">
                <div className="flex flex-col md:flex-row md:items-start gap-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-white mb-2 print:text-black">Analysis Complete.</h2>
                    ...
                    <div className="flex flex-wrap gap-4">
                      <div className="bg-gray-800/80 px-4 py-2 rounded-lg border border-gray-700 backdrop-blur-sm print:bg-gray-100 print:border-gray-300">
                        <span className="text-[10px] text-gray-400 uppercase tracking-wide block mb-1 print:text-gray-600">Monthly Surplus</span>
                        <span className={`text-xl font-bold text-emerald-400 print:text-black ${blurClass}`}>{aiResponse.surplus}</span>
                      </div>
                      <div className="bg-gray-800/80 px-4 py-2 rounded-lg border border-gray-700 backdrop-blur-sm print:bg-gray-100 print:border-gray-300">
                        <span className="text-[10px] text-gray-400 uppercase tracking-wide block mb-1 print:text-gray-600">Behavioral Profile</span>
            ...
            {/* ACTION GRID */}
            <div>
              <h3 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4 print:text-black">The Execution Plan</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 print:block">
              <div className="lg:col-span-2 print:mb-6">{aiResponse.mermaidCode && <MermaidChart chartCode={aiResponse.mermaidCode} />}</div>
              <div className="lg:col-span-1 bg-gray-900/50 p-6 rounded-xl border border-gray-700 mt-6 h-fit print:bg-white print:border-black print:text-black">
                <h4 className="text-xs font-bold text-gray-500 mb-4 uppercase tracking-wider print:text-black">Milestones</h4>
