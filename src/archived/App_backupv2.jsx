    ...
    if (chartCode && chartRef.current) {
      ...
      const renderChart = async () => {
        try {
          ...
          // SANITIZATION: Fix common AI Syntax errors before rendering
          // 1. Ensure space after colon
          // 2. Fix invalid dates if possible (basic regex)
          const cleanCode = chartCode
            .replace(/:([a-z])/g, ': $1') // Fix "task:crit" -> "task: crit"
            .replace(/crit1/g, 'crit');   // Fix "crit1" typo
  ...
  return (
    <div className="w-full bg-gray-900 border border-gray-700 rounded-xl mt-8 animate-fadeIn overflow-hidden shadow-2xl">
      ...
      <div className="overflow-x-auto p-6 bg-[#0B0F19]">
        {error ? (
          <div className="p-4 text-red-400 text-xs font-mono bg-red-900/10 rounded border border-red-500/30">
            <p className="font-bold mb-2">Visual Generation Error (Syntax):</p>
            <p>{error}</p>
            <div className="mt-4 pt-4 border-t border-red-500/30 text-gray-500">
              Raw AI Output:
              <pre className="mt-2 whitespace-pre-wrap">{chartCode}</pre>
            </div>
          </div>
        ) : (
